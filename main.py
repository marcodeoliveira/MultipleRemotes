...
client_rtu = ModbusClient(method='rtu', port=rtu_port, stopbits=rtu_stopbits, bytesize=rtu_bytesize, parity=rtu_parity, baudrate=rtu_baudrate, timeout1=0.050, retries=1)
...
def remote(weight,display):
	...
	if len(scale_unit) == 2:
		unit = scale_unit.ljust(3)
	else:
		unit = scale_unit.ljust(2)
	msg = "\x01\x02040200\x0201" + weight + "." + unit + "\x0202000000.   " + "\x0203" + weight + "." + unit + "\r\n"
	ser_dictionary["ser%s" %display].write(msg.encode())

# Loop for the RTU and execute transmission (200 ms)
try:
    while True:
      for port in [rtu_port]:
